...
describe('button happy path', () => {
  ...
  it('builds the MVP button through public commands', async () => {
    ...
    const [astro, scss, expectedScss] = await Promise.all([
    ...
    expect(astro).toContain('color && `button_color-${color}`')
    expect(astro).toContain("interface ButtonProps extends HTMLAttributes<'button'>")
    expect(astro).toContain("tag?: 'button' | 'div' | 'span'")
    expect(astro).toContain('href?: never')
    expect(astro).toContain("interface AnchorProps extends HTMLAttributes<'a'>")
    expect(astro).toContain("tag?: 'a'")
    expect(astro).toContain('href: string')
    expect(astro).toContain('type Props = (ButtonProps | AnchorProps) & {')
    expect(astro.match(/\btag[,=]/g)).toHaveLength(1)
    expect(astro).toContain('<slot>')
    expect(astro).toContain('<span class="button__icon">')
    expect(astro).toContain('<Icon name={iconStart}></Icon>')
    expect(astro).toContain('<span class="button__text">')
    expect(astro).toContain('{text}')
    expect(astro).not.toContain('button__icon-wrapper')
    expect(astro).toContain('<Icon name={iconEnd}></Icon>')
